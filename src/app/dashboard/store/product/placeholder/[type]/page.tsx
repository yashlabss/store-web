...
import PlaceholderProductTypeClient, {
  type PlaceholderSlug,
} from "../../../../../../components/store/PlaceholderProductTypeClient";
...
const SLUGS: PlaceholderSlug[] = ["coaching", "course", "community"];

function isPlaceholderSlug(s: string): s is PlaceholderSlug {
  return SLUGS.includes(s as PlaceholderSlug);
}

const REDIRECT_LOGIN =
  "/auth/login?redirectTo=/dashboard/store/product/placeholder/coaching";
...
function PlaceholderPageInner() {
  ...
  const raw = typeof params?.type === "string" ? params.type : "";
  const slug = useMemo(() => (isPlaceholderSlug(raw) ? raw : null), [raw]);
  ...
  useEffect(() => {
    if (!loading && raw && !isPlaceholderSlug(raw)) {
      router.replace("/dashboard/store/product/add");
    }
  }, [loading, raw, router]);
...
export default function PlaceholderProductTypePage() {
  return (
